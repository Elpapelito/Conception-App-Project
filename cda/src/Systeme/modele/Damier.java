...
public class Damier {
    ...
    public String nb_pions_blanc() {
        return String.valueOf(nb_pions_blanc);
    }
    ...
    public String nb_pions_noir() {
        return String.valueOf(nb_pions_noir);
    }
    ...
    public void tester_coup(String coup) throws Exception {
        if (!syntaxe_valide(coup))
            throw new Exception("Syntaxe invalide!\n");

        Map<Character, Integer> colonne = new HashMap<>();
        colonne.put('A', 0);colonne.put('B', 1);colonne.put('C', 2);
        colonne.put('D', 3);colonne.put('E', 4);colonne.put('F', 5);
    ...
    public boolean syntaxe_valide(String coup) {
        ...
        char b = coup.charAt(1);
        char c = coup.charAt(2);
        if (a < '1' || a > '8' || b != ' ' || c < 'A' || c > 'H')
            return false;
        return true;
    }

    public boolean coup_valide(int x, int y) {
        if(lesCases[x][y]!=0) return false;
        boolean b = false;
